...
const UI = {
  ...
  lostStat: document.getElementById('lostStat'),
...
function currentLanguage(){ return document.documentElement.lang==='ja'?'ja':'en'; }

function worldRuleLines(c, language=currentLanguage()){
  const isJa=language==='ja';
  const reaction=isJa
    ? {ignore:'マロを無視する',follow:'マロへ近づく',flee:'マロから離れる'}[c.maroReaction]
    : {ignore:'IGNORE MARO',follow:'MOVE TOWARD MARO',flee:'MOVE AWAY FROM MARO'}[c.maroReaction];
  const beacon=isJa?{0:'オフ',1:'中',2:'強'}[c.beaconPower]:{0:'OFF',1:'MEDIUM',2:'STRONG'}[c.beaconPower];
  return isJa ? [
    `${c.foodRate}秒ごと`, '→ 植物に食べ物が実る', '',
    'マンチャーが実った植物を見つけた', '→ 食べ物へ進む', '→ 食べる', '→ 空腹タイマーを0に戻す', '',
    `もし${MiniWorldRules.STARVATION_SECONDS}秒間 食べられなければ`, '→ マンチャーは死ぬ', '',
    'ドリフターがビーコンを見つけた', `→ 引く力 = ${beacon}`, '',
    'マロが近くにいる', `→ ${reaction}`, '',
    `クリーチャーの速さ = ${c.speedScale.toFixed(1)}×`
  ] : [
    `EVERY ${c.foodRate} SEC`, '→ PLANTS GROW FOOD', '',
    'MUNCHER SEES RIPE PLANT', '→ MOVE TO FOOD', '→ EAT', '→ RESET HUNGER TIMER', '',
    `IF NO FOOD FOR ${MiniWorldRules.STARVATION_SECONDS} SEC`, '→ MUNCHER DIES', '',
    'DRIFTER SEES BEACON', `→ PULL = ${beacon}`, '',
    'MARO IS NEAR', `→ ${reaction}`, '',
...
window.getSceneCode=()=>{
  ...
  return {
    title:ja?'ミニワールドのルール':'MINI WORLD RULES',
    steps:worldRuleLines(config()),
    operations:ja?[
      '空腹を更新(経過時間):\n  食べていない時間 += 経過時間\n  もし 30秒以上なら:\n    マンチャーを世界から取り除く',
      '食べる(植物):\n  植物を未成熟にする\n  空腹タイマー = 0\n  食べた数 += 1',
      '近いものを探す(対象):\n  すべての対象との距離を比べる\n  最も近い対象を返す'
    ]:[
      'UPDATE_HUNGER(delta):\n  time_without_food += delta\n  IF time_without_food >= 30 seconds:\n    REMOVE muncher from world',
      'EAT(plant):\n  plant.ripe = false\n  hunger_timer = 0\n  food_eaten += 1',
      'NEAREST(targets):\n  COMPARE distance to every target\n  RETURN closest target'
    ]
  };
};
...
function updateLabels(){
  const beaconNames=currentLanguage()==='ja'?['オフ','中','強']:['Off','Medium','Strong'];
...
function updateRulePreview(){
  UI.rulePreview.textContent=worldRuleLines(config()).join('\n');
}
...
function makeEntity(type,x,y){
  ...
  if(type==='muncher') return {...base, vx:rand(-40,40), vy:rand(-40,40), wanderTimer:0, secondsWithoutFood:0, phase:rand(0,6.28)};
...
function updateStats(){
  ...
  UI.lostStat.textContent=String(state.munchersLost);
}
...
function clearWorldQuiet(){
  state.entities=[]; state.elapsed=0; state.day=1; state.foodEaten=0; state.munchersLost=0;
  state.maro.x=480; state.maro.y=305; state.maro.vx=0; state.maro.vy=0;
}
...
function updateMuncher(m,dt,c,plants,ponds){
  ...
  if(!reactToMaro(m,speed,c.maroReaction)){
    ...
    if(near.item){
      ...
      if(near.distance<28){
        near.item.ripe=false; near.item.regrowTimer=c.foodRate; state.foodEaten++; m.secondsWithoutFood=0; log('A muncher found food.'); updateStats();
      }
    } else if(m.wanderTimer<=0){
      m.wanderTimer=rand(.7,1.5); const n=norm(rand(-1,1),rand(-1,1)); m.vx=n.x*speed*.7; m.vy=n.y*speed*.7;
    }
  }
  const hunger=MiniWorldRules.advanceHunger(m.secondsWithoutFood,dt);
  m.secondsWithoutFood=hunger.secondsWithoutFood;
  moveEntity(m,dt);
...
function update(dt){
  ...
  for(const e of state.entities){
    ...
    else if(e.type==='muncher'&&updateMuncher(e,dt,c,plants,ponds)) starved.push(e.id);
    else if(e.type==='drifter') updateDrifter(e,dt,c,beacons,ponds);
    else if(e.type==='beacon') e.phase+=dt*3;
  }
  if(starved.length){
    state.entities=state.entities.filter(e=>!starved.includes(e.id));
    state.munchersLost+=starved.length;
    starved.forEach(()=>log('A muncher starved after 30 seconds without food.'));
    updateStats();
  }
}
...
function drawMuncher(m){
  ...
  ctx.strokeStyle='#2b6de0';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-8,22);ctx.lineTo(-14,31+Math.sin(m.phase)*2);ctx.moveTo(8,22);ctx.lineTo(14,31-Math.sin(m.phase)*2);ctx.stroke();
  const foodLeft=clamp(1-m.secondsWithoutFood/MiniWorldRules.STARVATION_SECONDS,0,1);
  ctx.fillStyle='#fff9ef';ctx.strokeStyle='#13212d';ctx.lineWidth=2;ctx.fillRect(-25,-38,50,7);ctx.strokeRect(-25,-38,50,7);
  ctx.fillStyle=foodLeft<.34?'#ea5b43':'#6cad75';ctx.fillRect(-23,-36,46*foodLeft,3);ctx.restore();
}
...
window.addEventListener('oyc-language-change',()=>{
  updateLabels();
  UI.pauseBtn.textContent=state.paused?(currentLanguage()==='ja'?'再生':'Play'):(currentLanguage()==='ja'?'一時停止':'Pause');
});
