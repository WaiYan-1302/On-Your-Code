...
function updateRulePreview() {
  ...
  elements.rulePreview.textContent = [
    ...
    `→ ${BEHAVIOR_LABELS[form.nearBehavior].toUpperCase()}`,
    '',
    `OTHERWISE`,
    `→ ${BEHAVIOR_LABELS[form.idleBehavior].toUpperCase()}`,
    '',
...
function releaseCreature() {
  if (world.creatures.length >= 5) {
    window.alert('You can release up to 5 creatures in the lab.');
    return;
  }
    ...
    return `
      <article class="preset-item">
        <header>
          ...
          <span class="badge">${preset.bodyShape}</span>
        </header>
        <div>${BEHAVIOR_LABELS[preset.nearBehavior]} · ${BEHAVIOR_LABELS[preset.idleBehavior]}</div>
    ...
    return `
      <article class="roster-item" style="outline:${isSelected ? '3px solid #2B6DE0' : 'none'}">
        <header>
          ...
          <span class="badge ${isSelected ? 'blue' : ''}">${creature.bodyShape}</span>
        </header>
        <div>Near → ${BEHAVIOR_LABELS[creature.nearBehavior]}</div>
        <div>Idle → ${BEHAVIOR_LABELS[creature.idleBehavior]}</div>
        <div class="inline-buttons">
...
function drawCreature(creature) {
  const colors = COLORS[creature.colorStyle];
  const selected = creature.id === world.selectedCreatureId;
  ...
  if (selected) {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(0, 0, creature.size + 14, 0, Math.PI * 2);
    ctx.stroke();
  }
